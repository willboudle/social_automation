...
class Deven_Automation_Model_Facebook_Client {

    const REDIRECT_URI_ROUTE = 'adminhtml/facebook/authorize';

    const XML_PATH_ENABLED = 'automation/facebook/enabled';
    const XML_PATH_CLIENT_ID = 'automation/facebook/client_id';
    const XML_PATH_CLIENT_SECRET = 'automation/facebook/client_secret';
    const XML_PATH_REDIRECT_URI = 'automation/facebook/redirect_uri';
    ...
    const GRAPH_VIDEO_SERVICE_URI = 'https://graph-video.facebook.com';
    ...
    protected $scope = array('email', 'user_birthday',
        ...
        'manage_pages', 'user_managed_groups');
    ...
    public function debugToken()
    {
        ...
        $url = self::OAUTH2_SERVICE_URI.'/debug_token';
        $params = array('input_token' => $this->token->access_token);
        $params = array_merge(array(
            'access_token' => $this->token->access_token
        ), $params);

        $response = $this->_httpRequest($url, 'GET', $params);
    ...
    protected function _httpRequest($url, $method = 'GET', $params = array())
    {
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401)) {
                ...
                throw new Deven_Automation_FacebookOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('deven_automation')
...
class Deven_Automation_FacebookOAuthException extends Exception
{}